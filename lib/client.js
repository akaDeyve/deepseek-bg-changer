window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			// Loop guards: theme.overrideTokens() publishes a new snapshot and
			// PUBLISH EMITS "theme/change" — re-applying tokens inside that
			// listener recurses until the stack overflows and re-renders the
			// whole app thousands of times per settings change. The guards
			// below break that cycle.
			let lastTokensJson = "";
			let lastScheme = "";
			let pendingApply = 0;

			ctx.effect(() => () => {
				if (pendingApply) { clearTimeout(pendingApply); pendingApply = 0; }
			...
			// Re-pick the built-in gradient only when the color scheme actually
			// changed — never in response to our own token-layer publishes.
			ctx.on("theme/change", (snap) => {
				const scheme = snap && snap.active && snap.active.colorScheme ? snap.active.colorScheme : "";
			...
			function applyAll(s) {
				...
				// Apply the token layer ONLY when it actually changed. This is the
				// recursion breaker: an unchanged layer must not publish again,
				// because publish() emits "theme/change".
				const tokensJson = Object.keys(tokens).length ? JSON.stringify(tokens) : "";
			...
			// Storage-outcome hooks the settings section assigns on render.
			let notifySaveOk = null;
			let notifySaveFail = null;

			// Debounced apply + persist: the settings UI updates its state
			// instantly, while the expensive work (style/tag writes, token
			// layer publish, localStorage) runs once per 60 ms idle window
			// instead of on every slider/color input event.
			function scheduleApply(s) {
				...
				pendingApply = setTimeout(() => {
					...
					try {
						...
						if (notifySaveOk) notifySaveOk();
					} catch (e) {
						if (notifySaveFail) notifySaveFail();
					}
				}, 60);
			}
			...
			ctx.slots.inject("settings.section", () => ctx.slots.register(
				...
				(props) => {
					...
					// The debounced applier reports storage outcomes through these
					// hooks (assigned on render; only invoked later, from timers).
					notifySaveOk = () => setSaveNote("");
					notifySaveFail = () => setSaveNote("Auto-save failed (browser storage full?) — changes stay active until the next page load.");
